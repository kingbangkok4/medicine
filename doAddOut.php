...
$rows = $obj->read(" pd.product_ref_id = {$id} ");
if ($rows != false) {
    $row = $rows[0];
...
<html>
    ...
    <body>
        ...
        <div class="modal fade" tabindex="-1" role="dialog"  id="myModal" name="myModal">
          ...
          <form class="form-horizontal" role="post" method="post" action="index.php?viewName=out_product">
           ...
           <?php
		   		if($id  == "" || $quantity == "0" || $quantity == ""){
					...
					if($product_check_quantity < $quantity){
						...
						$product_refID = $obj->insert_product_out($data);
						if ($product_refID == false) {
							redirect("errorAddRecive.php.php");
							echo ($product_refID );
						} else {                          
							$obj->insert_product_detail_out($data, $_REQUEST["id"]);
